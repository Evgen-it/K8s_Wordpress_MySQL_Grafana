...
output "sg_mdb" {
  value = yandex_vpc_security_group.mdb.id
}
...
output "mdb_subnet_info" {
  value = [for k, v in var.subnets["mdb_mysql"] : zipmap(["subnet_id", "zone"], [yandex_vpc_subnet.subnet-main[v.name].id, yandex_vpc_subnet.subnet-main[v.name].zone])]
}
...
#output "lb_static_ip" {
  #value = yandex_dns_recordset.rs-a1.data
#}
